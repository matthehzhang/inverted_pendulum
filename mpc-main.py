...
def init():
    ...
    M = np.zeroes(2*horizon, 2) #each state takes 2 rows to match states (theta&v_angle)
                        #and 2 columns (effect of ea/ state from both theta0 & v_angle0)
    C = np.zeroes(2*horizon, horizon) #added effect of input to each state's properties

    


    return (g,L,m,theta,dot_theta,dt,horizon,total_steps,A,B,x,x_id,Ad,Bd,u,Q,R,Qf,u_max)

def mpc_step (x,horizon,Ad,Bd,Q,R,Qf,u_max):
...
def main():
    g,L,m,theta,dot_theta,dt,horizon,total_steps,A,B,x,x_id,Ad,Bd,u,Q,R,Qf,u_max = init()  
    ...
    for i in range(total_steps):
        u = mpc_step(x,horizon,Ad,Bd,Q,R,Qf,u_max)
